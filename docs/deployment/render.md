...
Render terminates HTTPS at its public web service layer before forwarding traffic to the container. The templates set `TRUSTED_PROXY_CIDRS=0.0.0.0/0,::/0` so Instatic trusts Render's forwarded host/protocol headers for CSRF origin checks, login session context, audit IPs, and rate-limit IPs. Use that broad value only for Render-managed services; custom reverse-proxy deployments should trust only their actual proxy CIDRs.
...
| First-run setup or login returns `Forbidden: invalid origin` | Confirm `TRUSTED_PROXY_CIDRS=0.0.0.0/0,::/0` is set so Instatic trusts Render's forwarded HTTPS/public-host headers. |
